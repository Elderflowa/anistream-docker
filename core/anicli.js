...
export async function getAnimeByAnilistId(anilistId, title) {
...
export async function getEpisodeUrls(allAnimeId, episodeNumber) {
  const streams = { sub: [], dub: [] };
  let subs = null;
  let refr = null;

  const subLines = await runAniCli(["-e", episodeNumber, allAnimeId]);
  // const dubLines = await runAniCli([
  //     "-e",
  //     episodeNumber,
  //     allAnimeId,
  //     "--dub",
...
export async function getSubtitles(allAnimeId, episodeNumber) {
  const subLines = await runAniCli(["-e", episodeNumber, allAnimeId]);
