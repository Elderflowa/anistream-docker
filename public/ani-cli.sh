...
play_episode() {
    ...
    # Output all available links
    # printf "\n\033[1;32m=== All Available Sources for %s Episode %s ===\033[0m\n\n" "$allanime_title" "$ep_no"
    printf "%s\n" "$links"
    update_history
}
...
play() {
    ...
    if [ "$line_count" != 1 ] || [ -n "$start" ]; then
        ...
        for i in $range; do
            ep_no=$i
            printf "\033[1;34mFetching sources for episode %s...\033[0m\n" "$ep_no"
            play_episode
        done
    else
        play_episode
...
case "$search" in
    ...
    *)
        if [ "$use_external_menu" = "0" ]; then
            while [ -z "$query" ]; do
                printf "\33[2K\r\033[1;36mEnter anime _id: \033[0m" && read -r query
            done
        else
            [ -z "$query" ] && query=$(printf "" | external_menu "" "Enter anime _id: " "$external_menu_args")
            [ -z "$query" ] && exit 1
        fi
